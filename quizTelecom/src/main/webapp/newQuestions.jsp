		...
		<div class="login-logo">
  		...
  		<div class="row">
 			 <div class="col-6" style="border: solid #333; margin-bottom:50px" >
				<form  action="questionsNewPost" method="post" class="container rounded" enctype="multipart/form-data">
...
<select class="form-control"  id="niveau" name="niveau">
      <c:forEach var="niveau" items="${niveaux}">
        <option value="${niveau.id}">${niveau.intitule}</option>
			...
			<div class="form-group">
		<label for="niveau">Media:</label>
			<input type="file" class="form-control" name="lien_pdf" required>
    
		</div>
			<div class="form-group">
		    <label for="theme">Theme:</label>
			<input type="text" class="form-control" name="theme" placeholder="Entrez votre theme" required>
		</div>
		...
		<c:forEach var="j" begin="1" end="4" step="1">
		  ...
		  <div class="form-check">
		   ...
		   <input type="number" name="idQuiz" value=${idQuiz} style="display: none;">
	 		...
	 		<button type="submit" class="btn btn-primary" style="background-color:#333; border:white;">Créer la question</button>
		  </div>
		</form>
